...
class rt_img:
    ...
    def mask(self, atlas):
        '''
        Masks real-time image to an atlas.
        Basically this returns clf_voxel_indices, but I can't figure out the best way to do it...

        Parameters
        ----------
        atlas : ???
            The atlas we are going to mask to. Should be str or nib.Nifti1Image, maybe?
        '''
        print('wip')
        clf_voxel_indices = None # should prob be named roi_voxels instead?
        return clf_voxel_indices
    ...
    def extract_roi_signals(self, roi_voxels):
        '''
        ...
        Parameters
        ----------
        roi_voxels : array-like of size (???)
            Generated by preproc.rt_img.mask() – WIP...
        '''
        # Generate some paths
        transformed_epi = self.proc_dir + '/img_mni_' + str(self.rep+1).zfill(3) + '.nii.gz'
